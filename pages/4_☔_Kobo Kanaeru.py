...
# Retrieve a DataFrame named 'df_kobo_st' from the Streamlit session state. This DataFrame is used for further analysis.
df_kobo = st.session_state.df_kobo_st
...
views = df_kobo['views'].iloc[-1]
views_delta = df_kobo['views'].iloc[-1] - df_kobo['views'].iloc[-2]

# calculate likes widget
likes = df_kobo['likes'].iloc[-1]
likes_delta = df_kobo['likes'].iloc[-1] - df_kobo['likes'].iloc[-2]

# calculate comments widget
comments = df_kobo['comments'].iloc[-1]
comments_delta = df_kobo['comments'].iloc[-1] - df_kobo['comments'].iloc[-2]

# calculate favorites widget
favorites = df_kobo['favorites'].iloc[-1]
favorites_delta = df_kobo['favorites'].iloc[-1] - df_kobo['favorites'].iloc[-2]
...
average_views = round((df_kobo['views'].iloc[-30:-1].sum() / len(df_kobo['views'].iloc[-30:-1])),2)
average_views_last = df_kobo['views'].iloc[-31:-2].sum() / len(df_kobo['views'].iloc[-31:-2])
average_views_delta = round(average_views - average_views_last,2)
#-----------------------------------------
average_likes = round((df_kobo['likes'].iloc[-30:-1].sum() / len(df_kobo['likes'].iloc[-30:-1])),2)
average_likes_last = df_kobo['likes'].iloc[-31:-2].sum() / len(df_kobo['likes'].iloc[-31:-2])
average_likes_delta = round(average_likes - average_likes_last,2)
#-----------------------------------------
average_comments = round((df_kobo['comments'].iloc[-30:-1].sum() / len(df_kobo['comments'].iloc[-30:-1])), 2)
average_comments_last = df_kobo['comments'].iloc[-31:-2].sum() / len(df_kobo['comments'].iloc[-31:-2])
average_comments_delta = round(average_comments - average_comments_last,2)
#-----------------------------------------
average_favorites = round((df_kobo['favorites'].iloc[-30:-1].sum() / len(df_kobo['favorites'].iloc[-30:-1])), 2)
average_favorites_last = df_kobo['favorites'].iloc[-31:-2].sum() / len(df_kobo['favorites'].iloc[-31:-2])
average_favorites_delta = round(average_favorites - average_favorites_last,2)

st.set_page_config(
    page_title='☔Kobo Kanaeru')

st.image("https://scontent.fbwx2-1.fna.fbcdn.net/v/t39.30808-6/277225148_107341411924275_6590773602962710296_n.jpg?stp=dst-jpg_p320x320&_nc_cat=111&ccb=1-7&_nc_sid=5f2048&_nc_ohc=jiA1XTagvvsAX-hjTFT&_nc_ht=scontent.fbwx2-1.fna&oh=00_AfDh4uLE7jyNjApsKSJCaobEdgl9BX0NHc9u1lsjvrN8oQ&oe=654C6327")

# set header
st.header('☔Kobo Kanaeru')
...
with tab1:   
    ...
    with col5:
        ...
        ax = sns.lineplot(data=df_kobo[-30:],x='date', y='views',color='blue',label='Videos Views')
        ...
        #-----------------------------------------        
        fig = plt.figure(figsize=(7,4))
        ax = sns.lineplot(data=df_kobo[-30:],x='date', y='comments',color='blue',label='Videos Comments')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Date")
        plt.ylabel('Comments')
    ...
    with col6:
        fig = plt.figure(figsize=(7,4))
        ax = sns.lineplot(data=df_kobo[-30:],x='date', y='likes',color='blue',label='Videos Likes')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Date")
        plt.ylabel('Likes')
        ...
        #-----------------------------------------        
        fig = plt.figure(figsize=(7,4))
        ax = sns.lineplot(data=df_kobo[-30:],x='date', y='favorites',color='blue',label='Videos Favorites')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Date")
...
with tab2:
    ...
    value = st.slider(label='Select Range Of Videos (Start : End)',
                       min_value=0, max_value=len(df_kobo), value=(0, len(df_kobo)))
    st.write('Total Videos Selected:', value[1]-value[0])
    st.subheader('Tag Percentages')
    value_counts = df_kobo[value[0]:value[1]]['tag_used'].value_counts()
    total_count = len( df_kobo[value[0]:value[1]])
    ...
    with col3:
        ...
        ax = sns.lineplot(data=df_kobo[value[0]:value[1]],x='date', y='views',color='blue')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Date")
        plt.ylabel("Views")
        ...
        fig = plt.figure(figsize=(9,5))
        ax = sns.lineplot(data=df_kobo[value[0]:value[1]],x='date', y='likes',color='blue')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Date")
        plt.ylabel("Likes")
        ...
        #----------------------------------------- 
        fig = plt.figure(figsize=(9,5))
        ax = sns.lineplot(data=df_kobo[value[0]:value[1]],x='date', y='comments',color='blue')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Date")
        plt.ylabel("Comments")
        ...
        #----------------------------------------- 
        fig = plt.figure(figsize=(9,5))
        ax = sns.lineplot(data=df_kobo[value[0]:value[1]],x='date', y='favorites',color='blue')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Date")
        plt.ylabel("Favorites")
    ...
    with col4:
        ...
        fig = plt.figure(figsize=(9,5))
        ax = sns.regplot(data=df_kobo[value[0]:value[1]],x=x_scale, y='views',color='blue',order=2)
        plt.ticklabel_format(style='plain', axis='y')
        ...
        x_scale = list(range(0, value[1]-value[0], 1))
        fig = plt.figure(figsize=(9,5))
        ax = sns.regplot(data=df_kobo[value[0]:value[1]],x=x_scale, y='likes',color='blue',order=2)
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Video index")
        plt.ylabel("Likes")
        plt.title("Videos Likes")
        ...
        x_scale = list(range(0, value[1]-value[0], 1))
        fig = plt.figure(figsize=(9,5))
        ax = sns.regplot(data=df_kobo[value[0]:value[1]],x=x_scale, y='comments',color='blue',order=2)
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Video index")
        plt.ylabel('Comments')
        plt.title("Videos Comments")
        ...
        x_scale = list(range(0, value[1]-value[0], 1))
        fig = plt.figure(figsize=(9,5))
        ax = sns.regplot(data=df_kobo[value[0]:value[1]],x=x_scale, y='favorites',color='blue')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Video index")
        plt.ylabel('Favorites')
        plt.title("Videos Favorites")
...
with tab3:
    ...
    x_axis_count_videos = list(range(len(df_kobo)))
    ...
    fig = plt.figure(figsize=(9,5))
    ax = sns.regplot(data=df_kobo,x=x_axis, y=y_axis,color='blue',order=2,label='Corelation metrics')
    plt.ticklabel_format(style='plain', axis='y')
...
with tab4:
    ...
    #-----------------------------------------
    df_kobo_spread = df_kobo[spread_axis.get(spread_metrics)]
    mesage_median = f"Median = {round(df_kobo_spread.median(),2)}"
    mesage_mean = f"Mean = {round(df_kobo_spread.mean(),2)}"
    modes = df_kobo_spread.mode(dropna=True)
    mesage_mode = f"Mode = {modes[0]}"
    mesage_std = f"Standard Deviation (STD)= {round(df_kobo_spread.std(),2)}"
    ...
    line_sale_median = plt.axvline(x = df_kobo_spread.median(), color = 'Blue', ls = '--', lw = 1.5, label = "Median")
    line_sale_mean = plt.axvline(x = df_kobo_spread.mean(), color = 'green', ls = '--', lw = 1.5, label = "Mean")
    line_sale_STD = plt.axvline(x = df_kobo_spread.std(), color = 'red', ls = '--', lw = 1.5, label = "STD")
    ## legend
    legend = plt.legend(handles = [line_sale_STD, line_sale_mean, line_sale_median], loc = 1)
    sns.histplot(data = df_kobo_spread, common_norm = True, color = '#b3afb2', edgecolor = '#1f041e').set(title=spread_title)
    plt.ticklabel_format(style='plain', axis='x')
    st.pyplot(fig)
...
with tab5:
    ...
    if n_number_all>0:
        ...
        df_kobo_all_time = df_kobo.sort_values(by=['views','likes','comments','favorites'],ascending=False).reset_index(drop=True)
        df_kobo_all_time = df_kobo_all_time[:n_number_all]
        value_counts = df_kobo_all_time['tag_used'].value_counts()
        total_count = len(df_kobo_all_time)
        ...
        st.dataframe(data=df_kobo_all_time[['views','likes','comments','favorites']+addon_collumns_all])
...
with tab6:
    ...
    if n_number>0:
        df_kobo_n = df_kobo[-n_number:].sort_values(by=['views','likes','comments','favorites'],ascending=False).reset_index(drop=True)
        value_counts = df_kobo_n['tag_used'].value_counts()
        total_count = len(df_kobo_n)
        ...
        st.dataframe(data=df_kobo_n[['views','likes','comments','favorites']+addon_n])
