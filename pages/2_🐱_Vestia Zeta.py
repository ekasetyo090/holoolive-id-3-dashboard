...
# Retrieve a DataFrame named 'df_zeta_st' from the Streamlit session state. This DataFrame is used for further analysis.
df_zeta = st.session_state.df_zeta_st
...
views = df_zeta['views'].iloc[-1]
views_delta = df_zeta['views'].iloc[-1] - df_zeta['views'].iloc[-2]

# calculate likes widget
likes = df_zeta['likes'].iloc[-1]
likes_delta = df_zeta['likes'].iloc[-1] - df_zeta['likes'].iloc[-2]

# calculate comments widget
comments = df_zeta['comments'].iloc[-1]
comments_delta = df_zeta['comments'].iloc[-1] - df_zeta['comments'].iloc[-2]

# calculate favorites widget
favorites = df_zeta['favorites'].iloc[-1]
favorites_delta = df_zeta['favorites'].iloc[-1] - df_zeta['favorites'].iloc[-2]
...
average_views = round((df_zeta['views'].iloc[-30:-1].sum() / len(df_zeta['views'].iloc[-30:-1])),2)
average_views_last = df_zeta['views'].iloc[-31:-2].sum() / len(df_zeta['views'].iloc[-31:-2])
average_views_delta = round(average_views - average_views_last,2)
#-----------------------------------------
average_likes = round((df_zeta['likes'].iloc[-30:-1].sum() / len(df_zeta['likes'].iloc[-30:-1])),2)
average_likes_last = df_zeta['likes'].iloc[-31:-2].sum() / len(df_zeta['likes'].iloc[-31:-2])
average_likes_delta = round(average_likes - average_likes_last,2)
#-----------------------------------------
average_comments = round((df_zeta['comments'].iloc[-30:-1].sum() / len(df_zeta['comments'].iloc[-30:-1])), 2)
average_comments_last = df_zeta['comments'].iloc[-31:-2].sum() / len(df_zeta['comments'].iloc[-31:-2])
average_comments_delta = round(average_comments - average_comments_last,2)
#-----------------------------------------
average_favorites = round((df_zeta['favorites'].iloc[-30:-1].sum() / len(df_zeta['favorites'].iloc[-30:-1])), 2)
average_favorites_last = df_zeta['favorites'].iloc[-31:-2].sum() / len(df_zeta['favorites'].iloc[-31:-2])
average_favorites_delta = round(average_favorites - average_favorites_last,2)


# set page configuration
st.set_page_config(
    page_title='🐱Vestia Zeta')

# Set a banner image at the top of the page.
st.image("https://scontent.fbwx2-1.fna.fbcdn.net/v/t39.30808-6/276151714_101849775817758_7168808745195956148_n.jpg?stp=dst-jpg_p320x320&_nc_cat=100&ccb=1-7&_nc_sid=5f2048&_nc_ohc=mgYpZmj8_hgAX8vbQlv&_nc_ht=scontent.fbwx2-1.fna&oh=00_AfCZ9tvvu-HKuVupyhwFUpFadQg9_y8vWppCWPMJmrn8aQ&oe=654B1972")

# set header
st.header('🐱Vestia Zeta')
...
with tab1:   
    ...
    with col5:
        ...
        ax = sns.lineplot(data=df_zeta[-30:],x='date', y='views',color='blue',label='Videos Views')
        ...
        #-----------------------------------------        
        fig = plt.figure(figsize=(7,4))
        ax = sns.lineplot(data=df_zeta[-30:],x='date', y='comments',color='blue',label='Videos Comments')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Date")
        plt.ylabel('Comments')
    ...
    with col6:
        fig = plt.figure(figsize=(7,4))
        ax = sns.lineplot(data=df_zeta[-30:],x='date', y='likes',color='blue',label='Videos Likes')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Date")
        plt.ylabel('Likes')
        ...
        #-----------------------------------------        
        fig = plt.figure(figsize=(7,4))
        ax = sns.lineplot(data=df_zeta[-30:],x='date', y='favorites',color='blue',label='Videos Favorites')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Date")
...
with tab2:
    ...
    value = st.slider(label='Select Range Of Videos (Start : End)',
                       min_value=0, max_value=len(df_zeta), value=(0, len(df_zeta)))
    st.write('Total Videos Selected:', value[1]-value[0])
    st.subheader('Tag Percentages')
    value_counts = df_zeta[value[0]:value[1]]['tag_used'].value_counts()
    total_count = len( df_zeta[value[0]:value[1]])
    ...
    with col3:
        ...
        ax = sns.lineplot(data=df_zeta[value[0]:value[1]],x='date', y='views',color='blue')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Date")
        plt.ylabel("Views")
        ...
        fig = plt.figure(figsize=(9,5))
        ax = sns.lineplot(data=df_zeta[value[0]:value[1]],x='date', y='likes',color='blue')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Date")
        plt.ylabel("Likes")
        ...
        #----------------------------------------- 
        fig = plt.figure(figsize=(9,5))
        ax = sns.lineplot(data=df_zeta[value[0]:value[1]],x='date', y='comments',color='blue')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Date")
        plt.ylabel("Comments")
        ...
        #----------------------------------------- 
        fig = plt.figure(figsize=(9,5))
        ax = sns.lineplot(data=df_zeta[value[0]:value[1]],x='date', y='favorites',color='blue')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Date")
        plt.ylabel("Favorites")
    ...
    with col4:
        ...
        fig = plt.figure(figsize=(9,5))
        ax = sns.regplot(data=df_zeta[value[0]:value[1]],x=x_scale, y='views',color='blue',order=2)
        plt.ticklabel_format(style='plain', axis='y')
        ...
        x_scale = list(range(0, value[1]-value[0], 1))
        fig = plt.figure(figsize=(9,5))
        ax = sns.regplot(data=df_zeta[value[0]:value[1]],x=x_scale, y='likes',color='blue',order=2)
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Video index")
        plt.ylabel("Likes")
        plt.title("Videos Likes")
        ...
        x_scale = list(range(0, value[1]-value[0], 1))
        fig = plt.figure(figsize=(9,5))
        ax = sns.regplot(data=df_zeta[value[0]:value[1]],x=x_scale, y='comments',color='blue',order=2)
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Video index")
        plt.ylabel('Comments')
        plt.title("Videos Comments")
        ...
        x_scale = list(range(0, value[1]-value[0], 1))
        fig = plt.figure(figsize=(9,5))
        ax = sns.regplot(data=df_zeta[value[0]:value[1]],x=x_scale, y='favorites',color='blue')
        plt.ticklabel_format(style='plain', axis='y')
        plt.xlabel("Video index")
        plt.ylabel('Favorites')
        plt.title("Videos Favorites")
...
with tab3:
    ...
    x_axis_count_videos = list(range(len(df_zeta)))
    ...
    fig = plt.figure(figsize=(9,5))
    ax = sns.regplot(data=df_zeta,x=x_axis, y=y_axis,color='blue',order=2,label='Corelation metrics')
    plt.ticklabel_format(style='plain', axis='y')
...
with tab4:
    ...
    #-----------------------------------------
    df_zeta_spread = df_zeta[spread_axis.get(spread_metrics)]
    mesage_median = f"Median = {round(df_zeta_spread.median(),2)}"
    mesage_mean = f"Mean = {round(df_zeta_spread.mean(),2)}"
    modes = df_zeta_spread.mode(dropna=True)
    mesage_mode = f"Mode = {modes[0]}"
    mesage_std = f"Standard Deviation = {round(df_zeta_spread.std(),2)}"
    ...
    line_sale_median = plt.axvline(x = df_zeta_spread.median(), color = 'Blue', ls = '--', lw = 1.5, label = "median")
    line_sale_mean = plt.axvline(x = df_zeta_spread.mean(), color = 'green', ls = '--', lw = 1.5, label = "mean")
    line_sale_STD = plt.axvline(x = df_zeta_spread.std(), color = 'red', ls = '--', lw = 1.5, label = "STD")
    ## legend
    legend = plt.legend(handles = [line_sale_STD, line_sale_mean, line_sale_median], loc = 1)
    sns.histplot(data = df_zeta_spread, common_norm = True, color = '#b3afb2', edgecolor = '#1f041e').set(title=spread_title)
    plt.ticklabel_format(style='plain', axis='x')
    st.pyplot(fig)
...
with tab5:
    ...
    if n_number_all>0:
        ...
        df_zeta_all_time = df_zeta.sort_values(by=['views','likes','comments','favorites'],ascending=False).reset_index(drop=True)
        df_zeta_all_time = df_zeta_all_time[:n_number_all]
        value_counts = df_zeta_all_time['tag_used'].value_counts()
        total_count = len(df_zeta_all_time)
        ...
        st.dataframe(data=df_zeta_all_time[['views','likes','comments','favorites']+addon_collumns_all])
...
with tab6:
    ...
    if n_number>0:
        df_zeta_n = df_zeta[-n_number:].sort_values(by=['views','likes','comments','favorites'],ascending=False).reset_index(drop=True)
        value_counts = df_zeta_n['tag_used'].value_counts()
        total_count = len(df_zeta_n)
        ...
        st.dataframe(data=df_zeta_n[['views','likes','comments','favorites']+addon_n])
